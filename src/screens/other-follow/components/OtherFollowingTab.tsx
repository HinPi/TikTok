...
export const OtherFollowingTab = memo((props: any): JSX.Element => {
  const { id } = props;
  const { response, loading } = useFetch(`${PATH.PROFILE}/following/${id}`);
  ...
  return response?.length === 0 ? (
    <View style={styles.container}>
      ...
      <TextField label="When this user starts following people, you will see them here" style={styles.contentStyles} />
    </View>
  ) : (
